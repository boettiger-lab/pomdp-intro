...
priors <- read_csv(file.path(log_dir, "observe_harvest_recruit/priors.csv"))
...
Figure 2 shows two separate policy curves for the same POMDP solution given two different prior belief distributions. The lower prior 
...
prior <- 
  ggplot(priors,aes(states, belief, color = prior)) + 
  geom_line(lwd=1)  + 
  scale_color_manual(values = colors) +
  coord_cartesian(xlim = c(0,1)) +
  ggtitle("POMDP priors") + 
...
gridExtra::grid.arrange(harvest, escapement, prior,
  layout_matrix = rbind(1,1,2,2, 3))
```
