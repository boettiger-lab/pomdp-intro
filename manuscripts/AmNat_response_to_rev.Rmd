...
We appreciate the careful and constructive comments of the editor and reviewers, which have allowed us to re-write the manuscript to both better communicate both the method and broader implications of the results shown here.  We also now address directly the significant differences between the extensive literature on the optimal management of fisheries to

We appreciate the time and effort of reviewer 1 in exploring these results in further simulation, but were unable to understand precisely what the reviewer had implemented. From the description provided, it appears the reviewer's results rather underscore our point about the importance of observation uncertainty, an uncertainty which does not appear to be fully present in the reviewer's simulation, as we discuss below.  In short, if the stock size is unknown (due to measurement uncertainty), how does one enforce the safe-guard that total allowable catch quota or harvest not exceed 90% the stock?  As our work shows, it precisely the presence of such assumptions 



We have expanded our results to include a comparisons to policies that much more closely resemble the implementation in actual fisheries



and believe our revisions should now address each of the concerns raised.  In particular, 
...
to achieve the target escapement exactly.  Overestimates of the stock size can lead to stock collapse

Without perfect measurements, the true escapement will vary under a  
...
The reviewer is entirely correct about this -- while a rigorous constant escapement policy shuts down
...
Moreover, many fisheries do not
...
We had no intention of suggesting that measurement uncertainty was under-represented in stock assessments; indeed it is precisely the *prevalance* with which stock assessment models account for uncertainty that motivates 
...
This is an excellent point which we now discuss properly. Our approach highlights that strategies such as Pretty Good Yield, taking 80% of the maximum sustainable yield, are not sufficient solutions to the challenge of measurement uncertainty, since they are still based on fundamentally deterministic premises.


As our analysis shows, unless it is possible to be certain beyond any shadow of a doubt that one has
...
Stock assessments give us 



Measurement errors accumulate.  

Pretty Good Yield, taking 80% of the maximum sustainable yield, reflects the same 


is defined as a percentage of maximum sustainable yield, which is an essentially deterministic concept.  It is impossible to be sure that 
