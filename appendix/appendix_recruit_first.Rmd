...
log_dir <- "../data/observe_recruit_harvest" # Store the computed solution files here
...
This uses a generic optimization routine to find the stock size at which the maximum
growth rate is achieved with respect to discount, i.e. $$\max_x \lbrace f(x) - x/\delta \rbrace$$
...
If we observe->recruit->harvest, $X_{t+1} = f(X_t) - h_t$, then we see the stock right after a harvest and before recuitment, when it is at $X_t ~ $B_{MSY}$.  
If we instead did observe->harvest->recruit, we would see the stock at its pre-harvest size, $X_t ~ B_{MSY} + $H_{MSY}$.  
...
```{r helper_routines}
## map cts values to nearest grid indices 
...
det_simulations <- function(x0 = 1/6, Tmax = 20, 
  ...
  det_policy <- function(x) pmax(f(x,0) - B_MSY,0) ## ASSUMES harvest takes place *after* recruitment
  ...
  list(msy = msy_policy, det = det_policy, pgy = pgy_policy) %>% 
    map_df(det_sim, f, x0, Tmax, .id = "method") 
}
```
...
pomdp_sims <- 
  map2_dfr(models[as.integer(meta$scenario)], alphas, function(.x, .y){
...
write_csv(policy_table, file.path(log_dir, "policy_table.csv"))
```
